...
unsigned int input_data_size, unsigned int output_data_size, unsigned int kernel_1_size, unsigned int kernel_2_size, unsigned int stride_1_size, unsigned int stride_2_size, unsigned int neurons_dense_1_size, unsigned int neurons_dense_2_size, unsigned int number_of_images)
{
	bench_t* aux_output_data = output_data;
    bench_t* aux_input_data = input_data;

	for(unsigned int position = 0; position < number_of_images; ++position)
    {
		aux_input_data = input_data + position * input_data_size * input_data_size;
        aux_output_data = output_data + position * output_data_size;

		// 1-1 Step convolution
		convolution(aux_input_data, conv_1_output, kernel_1_data, input_data_size, input_data_size, input_data_size, kernel_1_size);
		...
		// Softmax - Output
		softmax(dense_layer_2_output, aux_output_data, neurons_dense_2_size);
	}
}
