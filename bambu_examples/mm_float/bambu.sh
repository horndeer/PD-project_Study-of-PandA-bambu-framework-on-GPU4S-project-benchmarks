...
mkdir -p mm_float_synth_XSIM
cd mm_float_synth_XSIM
...
timeout 2h bambu $root_dir/module.c --clock-period=1.5 --std=c99 -s --top-fname=mm --generate-tb=$root_dir/test.xml \
                 ...
                 --channels-number=8 --experimental-setup=BAMBU-PERFORMANCE-MP --compiler=I386_CLANG12 "$@"
# timeout 2h bambu $root_dir/module.c --std=c99 --top-fname=mm --simulate --generate-tb=$root_dir/test.xml --evaluation \
#                   --simulate --simulator=XSIM --device-name=asap7-BC --experimental-setup=BAMBU-PERFORMANCE-MP "$@"
return_value=$?
